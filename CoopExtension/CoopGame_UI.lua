...
function create_UIBox_blind_choice(type, run_info)
	sendWarnMessage(" ========= create_UIBox_blind_choice =========", "MULTIPLAYER_COOP_EXTENSION")
	sendWarnMessage("type: " .. type, "MULTIPLAYER_COOP_EXTENSION")
	sendWarnMessage("G.LOBBY.code: " .. G.LOBBY.code, "MULTIPLAYER_COOP_EXTENSION")
	sendWarnMessage("G.LOBBY.gamemode: " .. G.LOBBY.config.gamemode, "MULTIPLAYER_COOP_EXTENSION")
	if G.LOBBY.code then
		-- If G.GAME.blind_on_deck is not already set, it initializes it to "Small".
		-- This likely represents the default or starting blind type.
		if not G.GAME.blind_on_deck then
			G.GAME.blind_on_deck = "Small"
		end

		-- If run_info is not provided, the blind state for the current blind (G.GAME.blind_on_deck) is set to "Select". 
		-- This likely indicates that the player needs to choose or interact with this blind.
		...
		if G.GAME.round_resets.blind_choices[type] == "bl_pvp" then
			local dt1 = DynaText({
				string = { { string = "TEAM UP!", colour = G.C.CHANCE } },
				colours = { G.C.BLACK },
				scale = 0.55,
				silent = true,
				pop_delay = 1,
		...
		{n = G.UIT.R,config = {id = type,align = "tm",func = "blind_choice_handler",minh = not run_info and 10 or nil,ref_table = { deck = nil, run_info = run_info },r = 0.1,padding = 0.05,},nodes = {
			{n = G.UIT.R,config = {align = "cm",colour = mix_colours(G.C.BLACK, G.C.L_BLACK, 0.5),r = 0.1,outline = 1,outline_colour = G.C.L_BLACK,},nodes = {
				{n = G.UIT.R,config = { align = "cm", padding = 0.2 },nodes ={
					{n = G.UIT.R,config ={id = "select_blind_button",align = "cm",ref_table = blind_choice.config,colour = run_info_colour,minh = 0.6,minw = 2.7,padding = 0.07,r = 0.1,shadow = true,hover = true,one_press = true,func = G.LOBBY.config.gamemode == "coop" and "pvp_ready_button"	or nil,button = "select_blind",},nodes ={
						{n = G.UIT.T,config ={	ref_table = G.GAME.round_resets.loc_blind_states,	ref_value = type,	text = localize(blind_state, "blind_states"),	scale = 0.45,	colour = G.C.UI.TEXT_LIGHT,	shadow = true,},},},}},},
...
G.FUNCS.blind_choice_handler = function(e)
    -- sendWarnMessage(" ========= blind_choice_handler =========", "MULTIPLAYER_COOP_EXTENSION")
    -- THIS SHIT IS SPAMMING WHEN IN MENU OF THE BLIND 
	if
...
G.FUNCS.pvp_ready_button = function(e)
    -- sendWarnMessage(" ========= pvp_ready_button =========", "MULTIPLAYER_COOP_EXTENSION")
    -- THIS SHIT IS SPAMMING WHEN IN MENU OF THE BLIND 
...
local function update_blind_HUD()
    sendWarnMessage(" ========= update_blind_HUD =========", "MULTIPLAYER_COOP_EXTENSION")
	if G.LOBBY.code then
...
local function reset_blind_HUD()
    sendWarnMessage(" ========= reset_blind_HUD =========", "MULTIPLAYER_COOP_EXTENSION")
	if G.LOBBY.code then
...
function G.FUNCS.mp_toggle_ready(e)
    sendWarnMessage(" ========= mp_toggle_ready =========", "MULTIPLAYER_COOP_EXTENSION")
...
function Game:update_draw_to_hand(dt)
    -- sendWarnMessage(" ========= update_draw_to_hand =========", "MULTIPLAYER_COOP_EXTENSION")
    -- Little spamming when at you draw a card from the deck
	if G.LOBBY.code then
		if
...
function Blind:defeat(silent)
    sendWarnMessage(" ========= Blind:defeat =========", "MULTIPLAYER_COOP_EXTENSION")
...
function Game:update_shop(dt)
    -- sendWarnMessage(" ========= update_shop =========", "MULTIPLAYER_COOP_EXTENSION")
...
local function eval_hand_and_jokers()
    sendWarnMessage(" ========= eval_hand_and_jokers =========", "MULTIPLAYER_COOP_EXTENSION")
...
function Game:update_hand_played(dt)
    sendWarnMessage(" ========= update_hand_played =========", "MULTIPLAYER_COOP_EXTENSION")
	-- Ignore for singleplayer or regular blinds
	if not G.LOBBY.connected or not G.LOBBY.code or not is_coop_boss() then
...
G.FUNCS.can_play = function(e)
    -- sendWarnMessage(" ========= can_play =========", "MULTIPLAYER_COOP_EXTENSION")
    -- THIS SHIT IS SPAMMING WHEN YOU HAVE YOUR HAND MENU ( READY TO PLAY A CARD )
...
function Game:update_new_round(dt)
    sendWarnMessage(" ========= update_new_round =========", "MULTIPLAYER_COOP_EXTENSION")
	...
	if G.LOBBY.code and not G.STATE_COMPLETE then
		...
		if to_big(G.GAME.chips) < to_big(G.GAME.blind.chips) and not is_coop_boss() then
...
function G.MULTIPLAYER.end_round()
    sendWarnMessage(" ========= G.MULTIPLAYER.end_round =========", "MULTIPLAYER_COOP_EXTENSION")
...
function Game:start_run(args)
    sendWarnMessage(" ========= start_run =========", "MULTIPLAYER_COOP_EXTENSION")
...
function create_UIBox_game_over()
    sendWarnMessage(" ========= create_UIBox_game_over =========", "MULTIPLAYER_COOP_EXTENSION")
	if G.LOBBY.code then
...
function create_UIBox_win()
    sendWarnMessage(" ========= create_UIBox_win =========", "MULTIPLAYER_COOP_EXTENSION")
	if G.LOBBY.code then
		G.SETTINGS.paused = false
...
function ease_ante(mod)
    sendWarnMessage(" ========= ease_ante =========", "MULTIPLAYER_COOP_EXTENSION")
...
function ease_lives(mod)
    sendWarnMessage(" ========= ease_lives =========", "MULTIPLAYER_COOP_EXTENSION")
	G.E_MANAGER:add_event(Event({
		trigger = "immediate",
		func = function()
...
function G.FUNCS:exit_overlay_menu()
    sendWarnMessage(" ========= exit_overlay_menu =========", "MULTIPLAYER_COOP_EXTENSION")
...
function G.FUNCS.mods_button(arg_736_0)
    sendWarnMessage(" ========= mods_button =========", "MULTIPLAYER_COOP_EXTENSION")
	if G.OVERLAY_MENU and G.OVERLAY_MENU:get_UIE_by_ID("username_input_box") ~= nil then
		G.MULTIPLAYER.UTILS.save_username(G.LOBBY.username)
	end
...
function reset_blinds()
    sendWarnMessage(" ========= reset_blinds =========", "MULTIPLAYER_COOP_EXTENSION")
	reset_blinds_ref()
	if G.LOBBY.code then
		-- if G.LOBBY.config.gamemode == "coop"
		-- then
		-- 	G.GAME.round_resets.blind_choices.Small = "bl_coop"
		-- 	G.GAME.round_resets.blind_choices.Big = "bl_coop"
		-- 	G.GAME.round_resets.blind_choices.Boss = "bl_coop"
		-- end
	end
end
...
function Game:update_selecting_hand(dt)
    -- sendWarnMessage(" ========= update_selecting_hand =========", "MULTIPLAYER_COOP_EXTENSION")
	-- THIS SHIT IS SPAMMING WHEN A HAND IS PLAYING
    if
...
G.FUNCS.can_open = function(e)
    -- sendWarnMessage(" ========= can_open =========", "MULTIPLAYER_COOP_EXTENSION")
	if G.MULTIPLAYER_GAME.ready_blind then
		e.config.colour = G.C.UI.BACKGROUND_INACTIVE
		e.config.button = nil
		return
...
function Blind:disable()
    sendWarnMessage(" ========= Blind:disable =========", "MULTIPLAYER_COOP_EXTENSION")
	if is_coop_boss() then
		return
	end
...
G.FUNCS.multiplayer_blind_chip_UI_scale = function(e)
    sendWarnMessage(" ========= multiplayer_blind_chip_UI_scale =========", "MULTIPLAYER_COOP_EXTENSION")
...
local function show_enemy_location()
    sendWarnMessage(" ========= show_enemy_location =========", "MULTIPLAYER_COOP_EXTENSION")
...
local function hide_enemy_location()
    sendWarnMessage(" ========= hide_enemy_location =========", "MULTIPLAYER_COOP_EXTENSION")
	local row_dollars_chips = G.HUD:get_UIE_by_ID("row_dollars_chips")
	if row_dollars_chips then
		row_dollars_chips.children[1]:remove()
		row_dollars_chips.children[1] = nil
...
function Game:update_blind_select(dt)
    -- sendWarnMessage(" ========= update_blind_select =========", "MULTIPLAYER_COOP_EXTENSION")
    -- THIS SHIT IS SPAMMING WHEN IN MENU OF THE BLIND 
	local updated_location = false
...
function G.FUNCS.select_blind(e)
    sendWarnMessage(" ========= select_blind =========", "MULTIPLAYER_COOP_EXTENSION")
	...
	-- send client startblind
end

--////////////////////////////////////////////////////////////////////////////////
local use_and_sell_buttons_ref = G.FUNCS.use_and_sell_buttons
function G.FUNCS.use_and_sell_buttons(card)
	sendWarnMessage(" ========= use_and_sell_buttons =========", "MULTIPLAYER_COOP_EXTENSION")
	use_and_sell_buttons_ref(card)
end

local card_focus_ui_ref = G.FUNCS.card_focus_ui
function G.FUNCS.card_focus_ui(card)
	sendWarnMessage(" ========= card_focus_ui =========", "MULTIPLAYER_COOP_EXTENSION")
	card_focus_ui_ref(card)
end

-- COULD BE USEFUL FOR P1 AND P2 SELECT CARD 
local create_UIBox_highlight_ref = create_UIBox_highlight
function create_UIBox_highlight(rect)
	sendWarnMessage(" ========= create_UIBox_highlight =========", "MULTIPLAYER_COOP_EXTENSION")
	return create_UIBox_highlight_ref(rect)
end